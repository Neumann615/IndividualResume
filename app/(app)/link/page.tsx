    ...
    return (
        <Container className={"lg:max-w-[1280px] w-auto"}>
            ...
            >
                    ...
                    {config.linkData.map((item, index) => {
                        return (
                            <div
                                ...
                                className={
                                    cn('xl:w-[200px] xl:h-14 xl:text-lg   text-sm h-a p-3 w-a  items-center flex  justify-center  mb-2 rounded-lg cursor-pointer duration-300', (currentIndex === index ? 'bg-blue-500 text-white dark:bg-blue-800' :
                ...
                <div
                    className="flex-1 grid gap-2 grid-cols-2 sm:grid-cols-3  sm:gap-3  md:grid-cols-4 md:gap-4 lg:grid-cols-4 lg:gap-4 xl:grid-cols-5 xl:gap-4">
                    {config.linkData[currentIndex].lists.map((item) => {
                        return (
                            ...
                            >
                                ...
                                <div
                                    className="text-center mb-2 h-[2.5rem] text-xs text-ellipsis line-clamp-2 text-zinc-400">
                                    {item.desc}
                                </div>
                                ...
                                <div className="flex justify-between items-center h-8">
                                    {item.home?.length && item.contribute?.length ? <>
                                            ...
                                            ></Github></>
                                        : <div className={'line-clamp-1 text-sm text-zinc-500'}>{item.home}</div>}
                                </div>
